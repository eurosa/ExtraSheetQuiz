...
public class RewardsAdapter extends RecyclerView.Adapter<RewardsAdapter.ViewHolder> {
    ...
    public RewardsAdapter(ArrayList<QuizRankRewardModel> r_list, ArrayList<QuizModel> quiz_list, Activity activity) {
        this.r_list = r_list;
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        QuizRankRewardModel r_model = r_list.get(position);
        holder.txt_amt.setText(r_model.getRewards());

        String j_id = r_model.getQuiz_id();
        String j_d = j_id.substring(4,12);
        String j_t = j_id.substring(12,j_id.length());
        ...
        try {
            ...
            holder.txt_time.setText("Date : "+date + "\nTime : "+tt);
        } catch (ParseException e) {
            e.printStackTrace();
        }
        holder.txt_ini.setText(r_list.get(position).getRank().toString());
      for (int i = 0 ; i<quiz_list.size();i++)
      {
          if (r_model.getQuiz_id().equals(quiz_list.get(i).getQuiz_id()))
          {
              holder.txt_name.setText(quiz_list.get(i).getTitle().toUpperCase());
//              holder.txt_name.setText(j_id);
    ...
    public void getQuizData(final String quiz_id , final TextView name , final TextView ini) {
